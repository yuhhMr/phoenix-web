<template>
  <div class="space-y-4">
    ...
    <DialogModal v-model:open="modalVisible" :title="isEdit ? '编辑角色' : '新增角色'" :loading="saving" @submit="save">
      <div class="space-y-4">
        <div>
...
const columns = [
  ...
  columnHelper.accessor('dataScope', {
    header: '数据范围',
    size: 140,
    cell: (info) => dataScopeMap[info.getValue()] || info.getValue(),
  }),
    ...
    cell: ({ row }) => {
      ...
      if (perm.hasPerm('system:role:delete')) {
        btns.push(
          h(
            'button',
            { class: 'text-sm text-red-500 hover:underline', onClick: () => remove(row.original.roleId) },
            '删除',
          ),
        )
      }
