<template>
  <div class="space-y-4">
    <div class="bg-surface rounded-xl p-4 shadow-sm flex flex-wrap items-center gap-4">
      ...
      <button class="px-4 py-2 border border-border rounded-md hover:bg-background" @click="resetQuery">
        重置
      </button>
    </div>
...
import { fetchRolePage, deleteRole, type RoleItem } from '@/api/role'
  ...
  columnHelper.display({
    ...
    cell: ({ row }) =>
      h('button', {
        ...
        onClick: () => remove(row.original.roleId),
      }, '删除'),
  }),
]
